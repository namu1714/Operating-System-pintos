...
/* Compares the value of two hash elements A and B. */
bool hash_less_function (const struct hash_elem *a, const struct hash_elem *b, void *aux){
  ...
  int a_data = a_item->data;
  int b_data = b_item->data;

  return (a_data < b_data);
}

/* Destruct and free hash element E. */
void hash_destructor (struct hash_elem *e, void *aux){
  struct hash_item *item = hash_entry(e, struct hash_item, elem);
  free(item);
}

/* square or triple hash element E. */
void hash_multiply (struct hash_elem *e, void *aux){
  int flag = *(int *)aux;
  struct hash_item *item = hash_entry(e, struct hash_item, elem);
  int value = item->data;
